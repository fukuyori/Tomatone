...
Inside a Git repository, the regular `go build -o tomatone.exe .` command also works. Use `-buildvcs=false` only if Go reports a VCS metadata detection error in a directory that is not under Git.
...
Open the generated `config.json` and edit the timer values and ambience URLs. This repository also includes [`config.example.json`](config.example.json) and the radio-oriented [`config.radio.example.json`](config.radio.example.json).
