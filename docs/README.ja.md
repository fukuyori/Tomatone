...
JSONを直接編集せずに音源の登録、修正、削除、試聴を行う場合は、CUI音源管理画面を開きます。
...
|---|---|
| `↑` / `↓` | 音源リストの選択を移動 |
| `a` | デフォルト、集中、休憩、または時間帯ルールのリストへURLを登録 |
| `e` | 選択中の音源URLを修正 |
| `m` | 選択中の音源の使用場面を変更し、移動先のリストを選択 |
| `d` | 選択中の音源を確認後に削除。確認は`y`／`n`を直接入力 |
| `Enter` / `p` | 選択中の音源を設定音量で試聴し、`Space`キーで停止 |
| `q` | 音源管理画面を終了 |

登録、修正、使用場面の変更、削除のたびに設定全体を検証し、設定ファイルへ即時保存します。既存の`time_rules`内の音源リストも登録先または変更先として選択できます。
上下キーによる選択と試聴停止には、即時キー入力を利用できるターミナルが必要です。標準入力をリダイレクトした場合は、従来のID指定による行入力に切り替わります。
